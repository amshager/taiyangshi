...
function renderGrid(viewDate) {
    ...
    if (UI.calTitleDate) {
        ...
        // 构造三历标题：公历 + 农历 + 彝历
        // 样式：字号统一，彝历为印章风格
        UI.calTitleDate.innerHTML = `${year}年${mStr}月 <span class="cal-title-sub">${viewLunar.monthName}</span> <span class="yi-title-stamp el-${yiData.wuxing}">${yiData.monthName}</span>`;
    }
...
export function updateCalendarView(date, lat, lon) { renderGrid(date); updateAstronomyData(date, lat, lon); }
